...
def _calc_A_D(n, vm, δ, εμ, k, fun, fun_der):
...
def _calc_A_N(n, vm, δ, εμ, k, fun, fun_der):
    A = zeros((len(vm), n, n), dtype=complex)
...
def _solve_2HD(prob, vm):
    ...
    if len(δ) == 1:
        ...
        nbu = 2 * len(δ) - 1
        A = _calc_A_D(nbu, vm, δ, prob.εμ, prob.k, prob.fun, prob.fun_der)
        F = _calc_F(nbu, vm, prob.k, δ[-1])
        return Solution(*prob, solve(A, F))


def _solve_2HN(prob, vm):
    δ = array(prob.radii)
    ...
    if len(δ) == 1:
        ...
        nbu = 2 * len(δ) - 1
        A = _calc_A_N(nbu, vm, δ, prob.εμ, prob.k, prob.fun, prob.fun_der)
        F = _calc_F(nbu, vm, prob.k, δ[-1])
        return Solution(*prob, solve(A, F))
...
def solve_prob(prob, M):
    m = arange(M + 1)
    if (prob.dim == 2) and prob.pde.startswith("H") and prob.type.startswith("D"):
        return _solve_2HD(prob, m)

    if (prob.dim == 2) and prob.pde.startswith("H") and prob.type.startswith("N"):
        return _solve_2HN(prob, m)

    if (prob.dim == 2) and prob.pde.startswith("H") and prob.type.startswith("P"):
